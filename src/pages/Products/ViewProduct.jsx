    ...
    <div className="w-full p-8 overflow-y-auto">
      <div className="border shadow-md border-gray-200 p-6 rounded-2xl flex flex-col gap-6">
        <div className="flex items-center gap-4">
          <img src={product.image_header} alt={product.name} className="w-12 h-12 rounded-md object-cover" />
          <div className="flex flex-col gap-1">
            <h1 className="text-2xl font-semibold text-black">{product.name}</h1>
            <p className="text-gray-500">{product.category}</p>
            <span className={`text-sm font-medium ${product.status === "In Stock" ? "text-green-600" : "text-red-500"}`}>{product.status}</span>
          </div>
        </div>
        <div className="flex gap-4 overflow-x-auto">
          {
            product.images.map((img, index) => (
                <img key={index} src={img} alt={`Preview ${index + 1}`} className="w-28 h-28 rounded-md object-cover border border-gray-200"/>
            ))
          }
        </div>
              ...
              <div className="flex flex-wrap gap-2">
                {
                  product.tags.map((tag, idx) => (
                      <span key={idx} className="bg-blue-100 text-blue-800 text-sm font-medium px-3 py-1 rounded-full">{tag}</span>
                  ))
                }
              </div>
            </div>
        ...
        <div className="flex flex-col gap-2">
          <h2 className="text-lg font-semibold text-gray-800">Detail Description</h2>
